...
import { contentItems, campaigns, automations, contacts } from '../data';
...
export default function ContentPage() {
  ...
  if (item) {
    ...
    const automation = automations.find(a => a.name === item.attachedAutomation);
            ...
            {automation && (
              <div className="pop-card p-5 pop-card-hover cursor-pointer">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className={`w-2 h-2 rounded-full ${automation.status === 'active' ? 'bg-[#10B981]' : 'bg-[#9B9B8F]'}`} />
                </div>
                <p className="text-[14px] font-semibold text-[#111111]">{automation.name}</p>
                <p className="text-[12px] text-[#6B6B6B]">{automation.trigger}</p>
              </div>
            )}
